...
/**
 * Zero-dependency inline SVG icons (Lucide-style strokes + brand marks).
 * Replaces the previous Font Awesome CDN stylesheet (~80KB of CSS + fonts)
 * with a few hundred bytes per icon, rendered server-side.
 */
@Component({
  selector: 'app-icon',
  ...
  template: `
    @switch (name()) {
      ...
      @case ('flask') {
        <svg [attr.width]="size()" [attr.height]="size()" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M10 2v7.5a2 2 0 0 1-.2.9L4.7 20.6a1 1 0 0 0 .9 1.4h12.8a1 1 0 0 0 .9-1.4L14.2 10.4a2 2 0 0 1-.2-.9V2M8.5 2h7M7 16h10"/>
        </svg>
      }
      @case ('gauge') {
        <svg [attr.width]="size()" [attr.height]="size()" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
